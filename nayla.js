...
• *Jawaban* : ${x31}`)
break 
case 'sticker': case 'stiker': case 's': case 'figuranha':
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
...
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
if (!q1 && !q2) return reply("Masukkan text1&text2\nContoh? #glitch aku&kamu")
reply("[❗] SEDANG DIPROSES")
nayla.sendMessage(from, {image:{url:`https://api.lolhuman.xyz/api/textprome2/${command}?apikey=Imannaz&text1=${q1}&text2=${q2}`}, caption:"👍", mentions:[sender]},{quoted:nay1})
break 
...
reply("[❗] SEDANG DIPROSES")
nayla.sendMessage(from, {image:{url:`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=Imannaz&text=${q}`}, caption:"👍", mentions:[sender]},{quoted:nay1})
break
...
if (!q) return reply("Masukkan Textnya 😇")
reply("[❗] SEDANG DIPROSES")
nayla.sendMessage(from, {image:{url:`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=Imannaz&text=${q}`}, caption:"👍", mentions:[sender]},{quoted:nay1})
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
if (!q) return reply("Masukkan Textnya 😇")
reply("[❗] SEDANG DIPROSES")
nayla.sendMessage(from, {image:{url:`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=Imannaz&text=${q}`}, caption:"👍", mentions:[sender]},{quoted:nay1})
...
if (budy == prefix + "sc"){reply("Private SC 😇")} // JANGAN DI UBAH:(
}} catch (err) {
